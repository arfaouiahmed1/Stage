...
def run_algorithms(X_scaled, df, distance_metric, features,
                   bandwidth=None, som_x=6, som_y=6,
                   sigma=1.0, learning_rate=0.5, num_iterations=100):
    results = []
    ...
    # DBSCAN
    dbscan = DBSCAN(metric=distance_metric if distance_metric != 'gower' else 'precomputed', eps=0.5, min_samples=5)
    if distance_metric == 'gower':
        dist_mat = gower.gower_matrix(df[features])
    ...
    try:
        ...
        results.append(("MiniSom", labels))
    except Exception as e:
        print(f"MiniSom failed: {e}")
...
def create_initial_groups(df, group_size_min=5, group_size_max=7):
    n_students = len(df)
    run_algorithmsn_groups = max(1, n_students // group_size_min)
...
features = ["gender", "nationality", "age", "hard_skills", "soft_skills", "creativity", "teamwork"]
bandwidth = None  # Optional: try a float like 1.5 if you want to tune it
som_x, som_y = 6, 6
sigma = 1.0
learning_rate = 0.5
num_iterations = 100
