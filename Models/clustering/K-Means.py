...
# --- Group settings ---
group_size = st.slider("Group size", min_value=2, max_value=10, value=5, step=1)
num_students = len(filtered_df)
num_groups = num_students // group_size
st.write(f"Total students: {num_students}, forming {num_groups} groups of {group_size}")
...
# --- Helper to compute average standard deviation
...
st.subheader(f"📋 Balanced Groups for class {selected_class}")
...
# --- Group Variance ---
st.subheader("⚖️ Average standard deviation per group (balance indicator)")
...
], columns=[f'Student {i+1}' for i in range(group_size)])
